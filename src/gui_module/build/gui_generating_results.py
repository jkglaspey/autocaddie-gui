...
def main(ser_out_ref = None, cameras_ref = None):
    ...
    def create_generating_results(frames_0, frames_1):
    ...
    def thread_sequencing_initial():
        ...
        if not confirm_status:
            window.after(0, wait_for_new_swing)
            return
        ...
        window.after(0, lambda: create_generating_results(frames_0, frames_1))
    ...
    def wait_for_new_swing():

        # Start receiving IMU data
        wait_for_ser = threading.Thread(target = wait_for_ser_out)
        wait_for_ser.start()

        # Update information
        nonlocal rectangle_2, rectangle_3, rectangle_1, text_1, button_1, button_2
        canvas.delete(rectangle_2)
        canvas.delete(rectangle_3)
        rectangle_2 = None
        rectangle_3 = None
    ...
    def wait_for_ser_out():
        global terminate_early, ser_out
        clear_buffer(ser_out)
        while terminate_early is False:
            ...
            # We got liftoff!
            if line == "READY":
                window.after(0, recording_window)

    def recording_window():
        # Start data recording process
        global cameras, ser_out
